...
int main(int argc, char** argv)
{
    ...
    double pi = TMath::Pi();
    double initial_sec = cal_sigma(m_dm/1000,m_med/1000); //g_chi=0.01; g_N = 0.01
    cout<<" ini xsec =  "<<initial_sec<<endl;
    string mass = argv[2];
    ...
    /////////////
    // input position of mountain
    gr_mountain = new TGraph2D("../dat/mountain_new.csv");
    gr_mountain->SetName("JinPingMountain");
    int N = gr_mountain->GetN();

    double center_long = 101.7;
    double center_lat = 28.18;
    double radius = 6371*1e3;
    double JP_alt = 1580;
    // transfer to the linear coordinate
    double *x_mountain = gr_mountain->GetX();
    double *y_mountain = gr_mountain->GetY();
    for(int i=0;i<N;i++){
        x_mountain[i] = (x_mountain[i]-center_long)/180.*pi*radius; //in m
        y_mountain[i] = (y_mountain[i]-center_lat)/180.*pi*radius; //in m
    }

    double xmin, xmax, ymin, ymax;
    const double xmin_mountain = gr_mountain->GetXaxis()->GetXmin();
    const double xmax_mountain = gr_mountain->GetXaxis()->GetXmax();
    const double ymin_mountain = gr_mountain->GetYaxis()->GetXmin();
    const double ymax_mountain = gr_mountain->GetYaxis()->GetXmax();
    double km_range = 25;
    if(km_range==0){
        xmin = xmin_mountain;//long
        xmax = xmax_mountain;//long
        ymin = ymin_mountain;//lat
        ymax = ymax_mountain;//lat
    } else {
        xmin = -km_range*1e3;
        xmax = km_range*1e3;
        ymin = -km_range*1e3;
    ...
    TF2* func_mountain = new TF2("mountain",myfunc_mountain,xmin,xmax,ymin,ymax);
    func_mountain->SetNpx(1000); func_mountain->SetNpy(1000);

    // Earth elements
    TH1D *Element = new TH1D("Element","Element",60,0,60);
    Element->SetBinContent(16+1,46.42);
    Element->SetBinContent(40+1,31.96);
    Element->SetBinContent(24+1,11.5);
    Element->SetBinContent(12+1,9.59);
    Element->SetBinContent(28+1,0.19);
    Element->SetBinContent(27+1,0.15);
    Element->SetBinContent(56+1,0.1);
    Element->SetBinContent(39+1,0.07);
    Element->SetBinContent(23+1,0.01);
    Element->SetBinContent(31+1,0.01);
    ...
    while(evtno<N_max){
        ...
        ////// input position
        x_before = gRandom->Uniform(xmin,xmax);
        y_before = gRandom->Uniform(ymin,ymax);
        z_before = func_mountain->Eval(x_before,y_before);
        ...
        while(T_dm_before>0.05 && counter<kMaxStep){    
            ...
            if(z[counter-1]>func_mountain->Eval(x[counter-1],y[counter-1])){
                f1->Delete();
                QE_input->Delete();
                QE_PDF->Delete();
                func_path->Delete();
            ...
            // The 1.5km*1.5km cicle selection
            else if(z[counter-1]<JP_alt){
                double x_cross, y_cross;
                if(counter>1){
                    x_cross = (JP_alt-z[counter-2])/(z[counter-1]-z[counter-2])*(x[counter-1]-x[counter-2])+x[counter-2];
                    y_cross = (JP_alt-z[counter-2])/(z[counter-1]-z[counter-2])*(y[counter-1]-y[counter-2])+y[counter-2];}
                else{
                    x_cross = (JP_alt-z_before)/(z[counter-1]-z_before)*(x[counter-1]-x_before)+x_before;
                    y_cross = (JP_alt-z_before)/(z[counter-1]-z_before)*(y[counter-1]-y_before)+y_before;}

                if(x_cross*x_cross+y_cross*y_cross<5e3*5e3){
                    // cout << "Arrived." << endl;
                    f1->Delete();
                    QE_input->Delete();
                    QE_PDF->Delete();
